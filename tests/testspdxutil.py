...
class TestSpdxUtil(unittest.TestCase):
    ...
    def test_createxlsx(self):
        ...
        ws = workbook.active
        self.assertEquals(len(TEST_PACKAGES), ws.max_row-1)
        for i in range(2, ws.max_row+1):
            pkg = TEST_PACKAGES[ws.cell(i, 1).value]
            self.assertIsNotNone(pkg)
            self.assertEquals(pkg['licenseDeclared'], ws.cell(i, 2).value)
            self.assertEquals(pkg['version'], ws.cell(i, 3).value)
            self.assertEquals(pkg['purl'], ws.cell(i, 4).value)
            self.assertEquals(pkg['dependency'], ws.cell(i, 5).value)
        # TEMP
