...
def analyzeFindingsInstancesForSubproject(cfg, prj, sp, slmJsonFilename):
    instances = []
    needReview = []

    # confirm whether this project has any findings templates
    if prj._findings == []:
        print(f'{prj._name}/{sp._name}: No findings template, skipping analysis')
        return [], []

    # get SLM JSON analysis details
    catLicFiles = loadSLMJSON(slmJsonFilename)
    if catLicFiles == []:
        print(f'{prj._name}/{sp._name}: Could not get any SLM category/license/file results; bailing')
        return None, None
...
# Helper for creating findings document, whether draft or final
...
def makeFindingsForSubproject(cfg, prj, sp, isDraft, includeReview=True):
    ...
    instances, needReview = analyzeFindingsInstancesForSubproject(cfg, prj, sp, slmJsonPath)
